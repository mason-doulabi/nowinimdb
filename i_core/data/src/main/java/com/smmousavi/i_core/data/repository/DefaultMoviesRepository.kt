...
class DefaultMoviesRepository @Inject constructor(
    ...
    override suspend fun getTypes(): Result<TypesModel> {
        return moviesRemoteDataSource.getTypes().fold(
    ...
    override suspend fun getGenres(): Result<GenresModel> {
        return moviesRemoteDataSource.getGenres().fold(
            onSuccess = { genres -> Result.success(genres.toDomain()) },
            onFailure = { error -> Result.failure(error) },
        )
    }

    override suspend fun getCountries(): Result<CountriesModel> {
        return moviesRemoteDataSource.getCountries().fold(
            onSuccess = { countries -> Result.success(countries.toDomain()) },
            onFailure = { error -> Result.failure(error) },
        )
    }

    override suspend fun getLanguages(): Result<LanguagesModel> {
        return moviesRemoteDataSource.getLanguages().fold(
